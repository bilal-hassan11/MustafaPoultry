        ...
        <div class="side-app">
            <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h4 class="mb-0">Edit Purchase Feed Return — Invoice #{{ $FeedInvoice[0]->invoice_no }}</h4>
                        ...
                        <div class="row">
                            <div class="col-md-2 mb-3">
                                ...
                                <input type="text" name="invoice_no" class="form-control"
                                       value="{{ $FeedInvoice[0]->invoice_no }}" readonly>
                            </div>
                            <div class="col-md-2 mb-3">
                                <label class="required">Date</label>
                                <input type="date" name="date" class="form-control"
                                       value="{{ $FeedInvoice[0]->date }}" required>
                            </div>
...
<script>
    // Products include the current invoice's own stock (ignored during edit-mode validation)
    let stockData       = {!! json_encode($products->values()->toArray()) !!};
    let existingItems   = {!! json_encode($FeedInvoice->toArray()) !!};
    ...
    $(document).ready(function () {
        ...
        $('#formData').submit(function (e) {
            ...
            $.ajax({
                ...
                data: $(this).serialize(),
                success: function () {
                    Swal.fire({ icon: 'success', title: 'Updated', text: 'Purchase Feed Return updated!' })
        ...
        $('body').on('input keyup blur', '.quantity, .purchaseRate, .dis_in_percentage', function () { Calculation(); });
