        ...
        <div class="side-app">
            <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h4 class="mb-0">
                        Edit Sale Medicine Return — Invoice #{{ $medicineInvoice[0]->invoice_no }}
                    </h4>
                    <a href="{{ route('admin.medicine-invoices.sale_return.index') }}"
                        ...
                        <table class="table table-bordered text-center" style="width: 100%">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: auto;">Purchase Rate</th>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            function addRow(item) {
                ...
                if (item.item_id) {
                    // change.select2 = visual update only; our data handler does NOT fire,
                    // preserving the saved sale_price / purchase_price values in the inputs.
                    $lastSelect.val(item.item_id).trigger('change.select2');
                }
                // Attach live-update handler AFTER initial value is set
            ...
            $("#formData").submit(function (e) {
                ...
                $.ajax({
                    url: "{{ route('admin.medicine-invoices.sale_return.store') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    success: function () {
                        Swal.fire({ icon: 'success', title: 'Updated',
                                    text: 'Sale Medicine Return updated!' })
                            .then(() => {
                                window.location.href = "{{ route('admin.medicine-invoices.sale_return.index') }}";
                            });
                    },
            ...
            $("body").on("input keyup blur", ".quantity, .saleRate, .dis_in_percentage",
